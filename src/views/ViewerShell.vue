...
<style scoped>
.viewer-shell { position: absolute; inset: 0; overflow: hidden; background: var(--c-bg); color: var(--c-text); }
...
.viewer-status h1 { margin: 0; color: var(--c-text-strong); font-size: 20px; letter-spacing: 0; }
.viewer-status p { margin: 0; color: var(--c-text-muted); }
.viewer-status button { margin-top: 8px; padding: 8px 13px; border: 1px solid var(--c-border-strong); border-radius: var(--radius-sm); background: var(--c-bg-elev); color: var(--c-text); font: inherit; cursor: pointer; }
.viewer-status button:hover { border-color: var(--c-accent); color: var(--c-accent); }
</style>