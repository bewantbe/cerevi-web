...
import { FUI_THEME, RulerOverlay, type State } from 'galavi'
...
describe('Galavi ruler overlay', () => {
  ...
  it('uses bar handles and defaults to the upper-right', () => {
    ...
    expect(bars[0]?.getAttribute('x1')).toBe('250')
    expect(bars[0]?.getAttribute('x2')).toBe('250')
    expect(bars[0]?.getAttribute('y1')).toBe('31.5')
    expect(bars[0]?.getAttribute('y2')).toBe('43.5')
    expect(bars[1]?.getAttribute('x1')).toBe('350')
    expect(bars[1]?.getAttribute('x2')).toBe('350')
  })
})